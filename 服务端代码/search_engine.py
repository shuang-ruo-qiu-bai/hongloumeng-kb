...
ORIGINAL_FILES = {
    "红楼梦脂评汇校本-cleaned.txt",
    "红楼梦 (曹雪芹  无名氏  程伟元  高鹗  中国艺术研究院红楼梦研究所)-cleaned.txt",
}

SOURCE_INFO: dict[str, str] = {
    "红楼梦脂评汇校本-cleaned.txt": "曹雪芹 著 · 脂砚斋 评 · 红楼梦原著（脂评汇校本）",
    "红楼梦 (曹雪芹  无名氏  程伟元  高鹗  中国艺术研究院红楼梦研究所)-cleaned.txt": "红楼梦原著 · 中国艺术研究院校注本",
    "红楼梦辨 (俞平伯)-cleaned.txt": "俞平伯 · 《红楼梦辨》 · 1923",
    "陈维昭：红学通史.md": "陈维昭 · 《红学通史》 · 上海人民出版社 2005",
    "红楼梦新证 (增订本) (周汝昌)-cleaned.txt": "周汝昌 · 《红楼梦新证》（增订本）",
    "红楼梦魇 (张爱玲)-cleaned.txt": "张爱玲 · 《红楼梦魇》 · 1977",
    "红楼梦探源 (吴世昌).pdf_by_PaddleOCR-VL-1.6.md": "吴世昌 · 《红楼梦探源》 · 牛津大学出版社 1961",
    "胡适红楼研究论述全编 (胡適).pdf_by_PaddleOCR-VL-1.5.md": "胡适 · 胡适红学论文集",
    "红楼梦诗词曲赋全解-cleaned.txt": "蔡义江 · 《红楼梦诗词曲赋全解》",
    "石头记索隐 蔡元培-cleaned.txt": "蔡元培 · 《石头记索隐》 · 1917",
    "红楼梦的两个世界 (余英时).pdf_by_PaddleOCR-VL-1.6.md": "余英时 · 《红楼梦的两个世界》 · 1974",
    ...
    "独上红楼 (梁归智).pdf_by_PaddleOCR-VL-1.5.md": "梁归智 · 《独上红楼》 · 探佚学专著",
    "红楼梦研究稀见资料汇编.pdf_by_PaddleOCR-VL-1.6.md": "红楼梦研究稀见资料汇编 · 早期红学文献汇编",
    "吴世昌 红楼碎墨 .pdf_by_PaddleOCR-VL-1.6.md": "吴世昌 · 《红楼碎墨》 · 红学论文随笔集",
    "红楼梦考证 (胡适)-cleaned.txt": "胡适 · 《红楼梦考证》单行本 · 1921",
    "红楼梦新解 (潘重规).pdf_by_PaddleOCR-VL-1.6.md": "潘重规 · 《红楼梦新解》 · 香港红学代表著作",
    "林黛玉家产与贾家谋夺说.md": "红学话题笔记 · 林黛玉家产与贾家谋夺说",
    "红楼梦藏书目录.txt": "红学藏书目录",
    "《红楼梦的法律世界》 (尹伊君著) -cleaned.txt": "尹伊君 · 《红楼梦的法律世界》 · 法律视角解读红楼梦",
    "命若朝霜：《红楼梦》里的法律、社会与女性 (柯岚) -cleaned.txt": "柯岚 · 《命若朝霜》 · 法律社会史视野下的红楼梦",
    "红楼梦大辞典 增订本.pdf_by_PaddleOCR-VL-1.5.md": "红楼梦大辞典 · 增订本",
    "红楼梦版本论 (林冠夫) .pdf_by_PaddleOCR-VL-1.6.md": "林冠夫 · 《红楼梦版本论》 · 版本研究专著",
}
...
def get_source_files():
    """Return list of indexed source files with metadata (excludes personal notes)."""
    files = []
    for rel_dir in ["raw/红楼梦", "topics"]:
...
def keyword_search(terms: str | list[str], root: Path | None = None,
                   any_term: bool = False, limit: int = 80,
                   original_only: bool = True) -> list[dict[str, Any]]:
    ...
    for base in search_dirs:
        ...
        for path in sorted(base.rglob("*")):
            if not path.is_file() or path.suffix.lower() not in {".txt", ".md", ".json"}:
                continue
            # When original_only, skip non-original files
            if original_only and path.name not in ORIGINAL_FILES:
                continue
            try:
...
def rag_search(query: str, top_k: int = 12, mode: str = "hybrid",
    ...
    for r in results[:top_k]:
        ...
        # Skip personal notes
        if source_path.startswith("notes/"):
            continue
        fname = Path(source_path).name
        # Filter: when original_only, skip non-original files
        if original_only and fname not in ORIGINAL_FILES:
            continue
        out.append({
            "text": r["text"][:2000],
            "source": source_path,
