#!/usr/bin/env python3
"""Hybrid RAG search for the 红楼梦研究 knowledge base.
...
# 如果本地有预先下载的模型，优先用本地路径
_LOCAL_MODEL = Path(__file__).resolve().parent.parent.parent / "embed_model"
if _LOCAL_MODEL.exists():
    EMBED_MODEL = str(_LOCAL_MODEL)
...
def print_status(collection, root: Path):
    ...
    db_path = root / CHROMA_DIR_NAME / "track_hongloumeng.db"
    if db_path.exists():
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Hybrid RAG search for 红楼梦知识库")
    parser.add_argument("query", nargs="?", help="Search query")
    parser.add_argument("--root", type=Path,
                        default=Path(os.environ.get("HLM_KB_ROOT",
    ...
    try:
        ...
        print("ERROR: Collection 'hongloumeng' not found. Run 'python3 scripts/rag_index.py' first.", file=sys.stderr)
        sys.exit(1)
