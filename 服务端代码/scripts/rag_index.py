#!/usr/bin/env python3
"""Index the 红楼梦研究 knowledge base into Chroma vector store.
...
def get_embed_fn():
    """Lazy-load embedding model with GPU acceleration if available."""
    import os
...
def get_track_path(root: Path) -> Path:
    return root / "chroma" / "track_hongloumeng.db"
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Index 红楼梦知识库 into Chroma")
    _script_dir = Path(__file__).resolve().parent.parent.parent
    parser.add_argument("--root", type=Path,
                        default=Path(os.environ.get("HLM_KB_ROOT", str(_script_dir))))
